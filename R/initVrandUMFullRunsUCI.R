...
#check which dataset to run FuzzyCrisp on
datasetname<-"iris"

myData<-read.csv(paste0(datafolder,datasetname,".csv") ,sep="\t",header=F,quote=NULL, comment="")
...
runresult=NULL
classresult=matrix(0, nrow=nrow(DM), ncol=runNum+1)
classeval<-NULL
...
#realignment of labels by permutation
